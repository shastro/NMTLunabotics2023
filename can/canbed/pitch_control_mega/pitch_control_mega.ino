...
void pack_telemetry(unsigned char buf[8], bool home_done) {
    ...
    data.left_position = david_pitch_position_telem_left_position_encode(
        (double)left_motor.count * MM_PER_COUNT);
    data.right_position = david_pitch_position_telem_right_position_encode(
        (double)right_motor.count * MM_PER_COUNT);
    ...
    data.in_home_state =
        david_pitch_position_telem_in_home_state_encode(in_home_state);
...
void home(MCP_CAN &can) {
    ...
    for (;;) {
        ...
        delay(50);
        if ((left_duplicates > required_num_duplicates) &&
            (right_duplicates > required_num_duplicates)) {
...
void setup() {
    ...
    // Interrupts
    attachInterrupt(digitalPinToInterrupt(HALL_PIN_L), left_hall_handler,
                    FALLING);
    attachInterrupt(digitalPinToInterrupt(HALL_PIN_R), right_hall_handler,
                    FALLING);
    ...
    for (;;) {
        ...
        if (current_command.home) {
            ...
            for (int i = 0; i < 5; i++) {
                CANPacket position_telemetry(
                    DAVID_PITCH_POSITION_TELEM_FRAME_ID);
