    ...
    while (true) {
        ...
        switch (n) {
            ...
            case 'J':
                sel_c = PITCH;
                if (sel_m != RIGHT) {
                    motorsys.pitch.length.left = std::min(motorsys.pitch.length.left + 64,
                            motorsys.pitch.length.max);
                } else if (sel_m != LEFT) {
                    motorsys.pitch.length.right = std::min(motorsys.pitch.length.right + 64,
                            motorsys.pitch.length.max);
                }
                break;
            case 'K':
                sel_c = PITCH;
                if (sel_m != RIGHT) {
                    motorsys.pitch.length.left = std::max(motorsys.pitch.length.left - 64,
                            motorsys.pitch.length.min);
                } else if (sel_m != LEFT) {
                    motorsys.pitch.length.right = std::max(motorsys.pitch.length.right - 64,
                            motorsys.pitch.length.min);
                }
                break;
            case 'o':
